    ...
    public class AdminReportService
    {
        ...
        public DataSet GetOrderReport(int companyId, DateTime startDate, DateTime endDate, string searchKey = null)
        {
            using (var connec = dapperContext.CreateConnection())
            {
                ...
                try
                {
                    string proce = "AdminReportModuleGetOrderReport";
        ...
        public DataSet GetProductReport(int companyId, DateTime startDate, DateTime endDate, string searchKey = null)
        {
            using (var connec = dapperContext.CreateConnection())
            {
                connec.Open();
                try
                {
                    string proce = "AdminReportModuleGetProductReport";
                    using (var cmd = new SqlCommand(proce, connec))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add(new SqlParameter
        ...
        public DataSet GetDayOfWeekReport(int companyId, DateTime startDate, DateTime endDate, string searchKey = null)
        {
            using (var connec = dapperContext.CreateConnection())
            {
                connec.Open();
                try
                {
                    string proce = "AdminReportModuleGetDayOfWeekReport";
                    using (var cmd = new SqlCommand(proce, connec))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add(new SqlParameter
                        ...
                        using (var adapter = new SqlDataAdapter(cmd))
                        {
                            var dataSet = new DataSet();
                            adapter.Fill(dataSet);
                            dataSet.Tables[0].TableName = "dayOfWeek";
                            return dataSet;
                        }
                    }
                }
